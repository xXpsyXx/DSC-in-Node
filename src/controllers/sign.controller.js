...
const svgToPngBuffer = async () => {
  ...
  try {
    ...
    throw new Error(
      `Failed to convert SVG to PNG: ${(error && error.message) || String(error)}`,
    );
  }
};
...
exports.getCertDetailsHandler = async (req, res) => {
  try {
    console.log(
      '[getCertDetailsHandler] incoming request headers:',
      req.headers || {},
    );
    console.log(
      '[getCertDetailsHandler] incoming content-type:',
      req.headers &&
    ...
    if (req.body) {
      try {
        console.log(
          '[getCertDetailsHandler] req.body present:',
    ...
    if (req.body && req.body.pin) {
      ...
      try {
        ...
        console.error(
          '[getCertDetailsHandler] formidable.parse error:',
          formErr && formErr.stack ? formErr.stack : formErr,
        );
        appendLog(
          'error',
          `form parse error: ${(formErr && formErr.message) || String(formErr)}`,
        );
        return res
          .status(400)
          .json({ error: 'Invalid form data or parse error' });
      }
    }
    ...
    appendLog(
      'error',
      `Unexpected error retrieving certificate details: ${(error && error.message) || String(error)}`,
    );
...
const addSignatureStampToPage = async (
  ...
  const boxX =
    typeof signatureX !== 'undefined' && signatureX !== null
      ? signatureX
      : defaultBoxX;
  const boxY =
    typeof signatureY !== 'undefined' && signatureY !== null
      ? signatureY
      : defaultBoxY;
...
const requestTsaTimestamp = async (hash) => {
  ...
  try {
    ...
    throw new Error(
      `Timestamp Authority (TSA) failed - PDFs must include cryptographic timestamps for legal validity. ${(tsaError && tsaError.message) || String(tsaError)}`,
    );
  }
};
...
exports.signHandler = async (req, res) => {
  ...
  try {
    const parsed = await form.parse(req);
    // Support promise-return style: parse may return [fields, files] or an object { fields, files }
    let fields = null;
    let files = null;
    if (Array.isArray(parsed)) {
      fields = parsed[0] || {};
      ...
      fields = parsed.fields || parsed[0] || {};
      files = parsed.files || parsed[1] || {};
    } else {
      fields = {};
      files = {};
    }
    // Uploaded file may be an array (formidable) or single object
    const uploadedFile = Array.isArray(files.file) ? files.file[0] : files.file;
    const pinRaw = fields.pin || (fields && fields['pin']);
    const pin = Array.isArray(pinRaw) ? pinRaw[0] : pinRaw;
    const driverPathRaw = fields.driverPath || (fields && fields['driverPath']);
    const driverPath = Array.isArray(driverPathRaw)
      ? driverPathRaw[0]
      : driverPathRaw;
    if (!uploadedFile)
    ...
    const sigXField = getFieldValue(fields, 'signatureX');
    const sigYField = getFieldValue(fields, 'signatureY');
    const sigXBody =
      req.body && typeof req.body.signatureX !== 'undefined'
        ? req.body.signatureX
        : undefined;
    const sigYBody =
      req.body && typeof req.body.signatureY !== 'undefined'
        ? req.body.signatureY
        : undefined;
    ...
    if (
      typeof sigXField !== 'undefined' &&
      sigXField !== null &&
      sigXField !== ''
    ) {
      signatureX = Number(sigXField);
      sigXSource = 'form';
    } else if (
      typeof sigXBody !== 'undefined' &&
      sigXBody !== null &&
      sigXBody !== ''
    ) {
      signatureX = Number(sigXBody);
      sigXSource = 'body';
    } else if (typeof payload.signatureX !== 'undefined') {
    ...
    if (
      typeof sigYField !== 'undefined' &&
      sigYField !== null &&
      sigYField !== ''
    ) {
      signatureY = Number(sigYField);
      sigYSource = 'form';
    } else if (
      typeof sigYBody !== 'undefined' &&
      sigYBody !== null &&
      sigYBody !== ''
    ) {
      signatureY = Number(sigYBody);
      sigYSource = 'body';
    } else if (typeof payload.signatureY !== 'undefined') {
    ...
    console.log(
      `[signHandler] Signature coords sources: X from ${sigXSource}, Y from ${sigYSource}`,
    );
    ...
    try {
      const expectedSerial =
        req.signPayload?.certSerial ||
        req.signPayload?.cert_serial ||
        req.signPayload?.expectedCertSerial;
      if (expectedSerial) {
        const certDetails = signer.getCertificateDetails();
...
exports.autoDetectTokenHandler = async (req, res) => {
  try {
    ...
    appendLog(
      'error',
      `Auto-detect failed: ${(error && error.message) || String(error)}`,
    );
    res
      .status(500)
      .json({ detected: false, error: 'Failed to auto-detect USB token' });
  }
};
