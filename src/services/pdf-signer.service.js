class PdfSignerService {
  static createPkcs7Signature(_pdfBytes, rsaSignatureBase64, _certificatePem, _certificateDer) {
    try {
      return Buffer.from(rsaSignatureBase64, 'base64').toString('hex');
    } catch (error) {
      console.error('[PdfSignerService] Error creating PKCS#7 signature:', error);
      throw new Error(`Failed to create PKCS#7 signature: ${(error && error.message) || String(error)}`);
    }
  }

  static embedDetachedSignatureBlock(pdfBytes, options) {
    ...
    const reason = this.escapePdfLiteral(options.reason || 'Digitally signed with Hypersecu USB token');
    ...
    blockLines.push('>>', 'endobj', `%%DSC_SIG_REF /Sig ${signatureObjectNumber} 0 R`, '%%DSC_SIG_BLOCK_END', '');
  ...
  static extractDetachedSignatureBlock(pdfBytes) {
    const pdfText = pdfBytes.toString('binary');
    ...
    while (blockEndExclusive < pdfText.length && (pdfText[blockEndExclusive] === '\n' || pdfText[blockEndExclusive] === '\r')) {
      blockEndExclusive += 1;
    }
    ...
    const certMatch = blockText.match(/\/DSCCertificate\s*\(([A-Za-z0-9+/=]+)\)/);
    ...
    if (certMatch?.[1]) {
      try {
        ...
        console.warn('[PdfSignerService] Failed to decode embedded certificate:', e);
      }
    }
    ...
    if (certificatePem) result.certificatePem = certificatePem;
    if (hmacMatch?.[1]) result.serverHmac = hmacMatch[1];

    return result;
  }

  static removeDetachedSignatureBlock(pdfBytes, blockStart, blockEndExclusive) {
    return Buffer.concat([pdfBytes.subarray(0, blockStart), pdfBytes.subarray(blockEndExclusive)]);
  }
  ...
  static escapePdfLiteral(value) {
    return value.replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');
  }

  static decodePdfLiteral(value) {
    return value.replace(/\\\(/g, '(').replace(/\\\)/g, ')').replace(/\\\\/g, '\\').trim();
  }
}

module.exports = { PdfSignerService };
