...
class SignerService {
  constructor(pin, customDriverPath) {
    ...
    if (!pkcs11LibraryPath) {
      throw new Error('USB token library not configured. Set PKCS11_LIBRARY_PATH (or PKCS11_LIBRARY_PATH_WINDOWS).');
    }
  ...
  getCertificatePem() {
    if (!this.certificateDer) return null;
    const cert = forge.pki.certificateFromAsn1(forge.asn1.fromDer(this.certificateDer.toString('binary')));
    return forge.pki.certificateToPem(cert);
  }

  getCertificateExpirationStatus() {
    if (!this.certificateDer) {
      return { status: 'valid', daysRemaining: 365, expiryDate: new Date(), message: 'Certificate not available' };
    }
    const cert = forge.pki.certificateFromAsn1(forge.asn1.fromDer(this.certificateDer.toString('binary')));
  ...
  initializePkcs11(libraryPath, pin) {
    ...
    try {
      ...
      try { pkcs11.C_Login(session, pkcs11js.CKU_USER, pin); } catch (error) {
        const message = this.getErrorMessage(error);
        if (!message.includes('CKR_USER_ALREADY_LOGGED_IN')) throw error;
      }
      const privateKey = this.findPrivateKey(session);
      if (!privateKey) throw new Error('No signing key found on USB token. Set PKCS11_KEY_LABEL or PKCS11_KEY_ID_HEX if your token has multiple keys.');
      this.pkcs11PrivateKey = privateKey;
      this.signerName = this.resolveSignerNameFromToken(session, privateKey);
    } catch (error) {
      this.close();
      throw new Error(`PKCS#11 initialization failed: ${this.getErrorMessage(error)}`);
    }
  }

  resolvePkcs11LibraryPath() {
    if (this.customDriverPath && String(this.customDriverPath).trim()) return this.customDriverPath.trim();
  ...
  static getSupportedDrivers() {
    const platform = os.platform();
    const drivers = [
      { name: 'Hypersecu ePass3000', path: platform === 'win32' ? 'C:\\Windows\\System32\\eps2003csp11v2.dll' : '/usr/lib/libepass2003.so', enabled: true },
      { name: 'SafeNet eToken', path: platform === 'win32' ? 'C:\\Windows\\System32\\eTPKCS11.dll' : '/usr/lib/libeTPKCS11.so', enabled: false },
      { name: 'Thales Luna USB', path: platform === 'win32' ? 'C:\\Windows\\System32\\lunacsp.dll' : '/usr/lib/liblunacsp.so', enabled: false },
      { name: 'YubiKey 5', path: platform === 'win32' ? 'C:\\Windows\\System32\\ykcs11.dll' : '/usr/lib/libykcs11.so', enabled: false },
      { name: 'Gemalto IDGo 800', path: platform === 'win32' ? 'C:\\Windows\\System32\\gclib.dll' : '/usr/lib/libgclib.so', enabled: false },
    ];
    return drivers;
  }
  ...
  static autoDetectDriver() {
    ...
    for (const driver of drivers) {
      try {
        ...
        try { pkcs11.C_Finalize(); } catch {}
        console.log(`[autoDetectDriver] Detected device: ${driver.name} (${driver.path})`);
        return { driverPath: driver.path, driverName: driver.name };
      } catch (error) {
  ...
  static probeTokenInfo() {
    ...
    for (const driver of drivers) {
      try {
        ...
        pkcs11.C_Initialize();
        const slots = pkcs11.C_GetSlotList(true);
        if (!slots || slots.length === 0) { try { pkcs11.C_Finalize(); } catch {} ; continue; }
        const slot = slots[0];
        if (!slot) { try { pkcs11.C_Finalize(); } catch {} ; continue; }
        let info = {};
        try { info = pkcs11.C_GetTokenInfo(slot) || {}; } catch (e) { info = {}; }
        try { pkcs11.C_Finalize(); } catch {}
        const tokenInfo = {};
        const labelVal = info.label ? (Buffer.isBuffer(info.label) ? info.label.toString('utf8').trim() : String(info.label)) : undefined;
        ...
        const modelVal = info.model ? (Buffer.isBuffer(info.model) ? info.model.toString('utf8').trim() : String(info.model)) : undefined;
        if (modelVal) tokenInfo.model = modelVal;
        const serialVal = info.serialNumber ? (Buffer.isBuffer(info.serialNumber) ? info.serialNumber.toString('utf8').trim() : String(info.serialNumber)) : undefined;
        if (serialVal) tokenInfo.serialNumber = serialVal;
        return { driverPath: driver.path, driverName: driver.name, tokenInfo };
      } catch (error) {
        // try next driver
      }
    }
  ...
  requirePkcs11() {
    if (!this.pkcs11) throw new Error('PKCS#11 is not initialized');
    return this.pkcs11;
  }
  ...
  getAttributeValue(session, objectHandle, attributeType) {
    const pkcs11 = this.requirePkcs11();
    const attrs = pkcs11.C_GetAttributeValue(session, objectHandle, [{ type: attributeType }]);
    const matched = attrs.find((attr) => attr.type === attributeType);
    if (!matched) return null;
    return matched.value;
  }

  findPrivateKey(session) {
    ...
    if (keyLabel) template.push({ type: pkcs11js.CKA_LABEL, value: keyLabel });
    if (keyIdHex) template.push({ type: pkcs11js.CKA_ID, value: Buffer.from(keyIdHex, 'hex') });
    let handle = this.findSingleObject(session, template);
    if (!handle && (keyLabel || keyIdHex)) {
      handle = this.findSingleObject(session, [{ type: pkcs11js.CKA_CLASS, value: pkcs11js.CKO_PRIVATE_KEY }]);
    }
    return handle;
  }

  resolveSignerNameFromToken(session, privateKey) {
    const keyId = this.getAttributeValue(session, privateKey, pkcs11js.CKA_ID);
    const certificateTemplate = [{ type: pkcs11js.CKA_CLASS, value: pkcs11js.CKO_CERTIFICATE }];
    if (keyId) certificateTemplate.push({ type: pkcs11js.CKA_ID, value: keyId });
    let certHandle = this.findSingleObject(session, certificateTemplate);
    if (!certHandle) certHandle = this.findSingleObject(session, [{ type: pkcs11js.CKA_CLASS, value: pkcs11js.CKO_CERTIFICATE }]);
    if (!certHandle) return 'Hypersecu Token User';
    const certDer = this.getAttributeValue(session, certHandle, pkcs11js.CKA_VALUE);
    const certLabel = this.getAttributeValue(session, certHandle, pkcs11js.CKA_LABEL)?.toString('utf8').trim();
    if (!certDer) return certLabel || 'Hypersecu Token User';
    ...
    try {
      ...
      const subjectDn = cert.subject.attributes.map((attr) => `${attr.shortName || attr.name}=${attr.value}`).join(', ');
  ...
  getTokenInfo() {
    ...
    try {
      ...
      const tokenInfo = {};
      const label = this.normalizeAttrValue(info.label) || this.normalizeAttrValue(info.manufacturerID) || this.normalizeAttrValue(info.tokenLabel);
      if (label) tokenInfo.label = label;
      const manufacturer = this.normalizeAttrValue(info.manufacturerID) || this.normalizeAttrValue(info.manufacturerId);
  ...
  getCertificateDetails() {
    if (!this.certificateDer) return { ownerName: null, serialNumber: null, expiryDate: null };
    try {
      const certAsn1 = forge.asn1.fromDer(this.certificateDer.toString('binary'));
      const cert = forge.pki.certificateFromAsn1(certAsn1);
      const commonName = cert.subject.getField('CN')?.value || null;
      const serialNumber = cert.serialNumber || null;
      const expiryDate = cert.validity?.notAfter ? cert.validity.notAfter : null;
  ...
  signHash(hashHex) {
    ...
    const pkcs11 = this.requirePkcs11();
    if (!session || !privateKey) throw new Error('PKCS#11 session is not ready for signing');
